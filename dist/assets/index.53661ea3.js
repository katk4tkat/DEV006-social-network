...
function home(navigateTo2) {
  ...
  footer.textContent = "Copyright \xA9 2023 Isabel Lira, Kat Bravo & Alejandra Mart\xEDnez";
 ...
 * limitations under the License.
 */
class GoogleAuthProvider$1 extends BaseOAuthProvider {
  constructor() {
    super("google.com");
  ...
  static credential(idToken, accessToken) {
    return OAuthCredential._fromParams({
      providerId: GoogleAuthProvider$1.PROVIDER_ID,
      signInMethod: GoogleAuthProvider$1.GOOGLE_SIGN_IN_METHOD,
      idToken,
      accessToken
    });
  }
  static credentialFromResult(userCredential) {
    return GoogleAuthProvider$1.credentialFromTaggedObject(userCredential);
  }
  static credentialFromError(error) {
    return GoogleAuthProvider$1.credentialFromTaggedObject(error.customData || {});
  }
  static credentialFromTaggedObject({ _tokenResponse: tokenResponse }) {
    if (!tokenResponse) {
      return null;
    ...
    try {
      return GoogleAuthProvider$1.credential(oauthIdToken, oauthAccessToken);
    } catch (_a) {
      return null;
    }
  }
...
const db = Ph(app);
const colRef = _h(db, "Posts");
console.log(db);
const deletePost = (id2) => yf(gh(db, "Posts", id2));
...
function wall(navigateTo2) {
  ...
  inputTittlePost.setAttribute = "textarea";
  inputPost.setAttribute = "textarea";
  let editStatus = false;
  let id2 = "";
  window.addEventListener("DOMContentLoaded", async () => {
    If(colRef, (querySnapshot) => {
      ...
      querySnapshot.forEach((doc) => {
        ...
        editBtn.addEventListener("click", async (e) => {
          ...
          const doc2 = await getPost(e.target.dataset.id);
          const editPostData = doc2.data();
